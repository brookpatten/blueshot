...
using blueshot.Forms;
using blueshot.Native;
using log4net;
        ...
        private void MakeCapture()
        {
            ...
            switch (_captureMode)
            {
                ...
                case CaptureMode.File:
                    ...
                    {
                        // TODO: Fix that the blueshot format needs a separate code path
                        try
                        {
                            if (filename.ToLower().EndsWith("." + OutputFormat.blueshot))
                            {
                                ISurface surface = new Surface();
                                surface = ImageIO.LoadblueshotSurface(filename, surface);
        ...
        public static ICapture CaptureWindow(WindowDetails windowToCapture, ICapture captureForWindow, WindowCaptureMode windowCaptureMode)
        {
            ...
            using (Process process = windowToCapture.Process)
            {
                ...
                if (isAutoMode)
                {
                    ...
                    // In https://github.com/blueshot/blueshot/issues/373 it was shown that PrintWindow (GDI) works great with Windows 11
                    // In https://github.com/blueshot/blueshot/issues/658 is was made clear it DOESN'T!
