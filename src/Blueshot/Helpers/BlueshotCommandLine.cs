...
// printing help text, because blueshot is built as a WinExe and has no console by default.
using Dapplo.Windows.Kernel32;

namespace blueshot.Helpers
{
    /// <summary>
    /// Holds the result of parsing blueshot's command line arguments.
    /// </summary>
    public class CommandLineOptions
    {
        /// <summary>
        /// When true, send an exit command to all running blueshot instances and exit.
        /// </summary>
        public bool Exit { get; set; }

        /// <summary>
        /// When true, send a reload-configuration command to all running blueshot instances and exit.
        /// </summary>
        ...
        /// <summary>
        /// When set, use this directory for reading and writing the blueshot.ini configuration file.
        /// </summary>
        public string IniDirectory { get; set; }

        /// <summary>
        /// One or more image files to open in the running blueshot instance,
    ...
    /// <summary>
    /// Defines and parses blueshot's command line interface using System.CommandLine.
    ...
    /// </summary>
    internal static class blueshotCommandLine
    {
        private static readonly Option<bool> ExitOption = new Option<bool>("--exit")
        {
            Description = "Send an exit command to all running blueshot instances."
        };

        private static readonly Option<bool> ReloadOption = new Option<bool>("--reload")
        {
            Description = "Send a reload-configuration command to all running blueshot instances."
        };

        private static readonly Option<bool> NoRunOption = new Option<bool>("--no-run")
        {
            Description = "Exit immediately without starting or showing blueshot."
        };

        private static readonly Option<string> LanguageOption = new Option<string>("--language")
        {
            HelpName = "language-code",
            Description = "Set the UI language for blueshot (e.g. en-US, de-DE) and save the configuration."
        };

        private static readonly Option<string> IniDirectoryOption = new Option<string>("--ini-directory")
        {
            HelpName = "directory",
            Description = "Set the directory where blueshot.ini is stored and read."
        };
        ...
        private static readonly Argument<string[]> FilesArgument = new Argument<string[]>("files")
        {
            ...
            Description = "One or more image files to open. If blueshot is already running, the files are opened in the existing instance."
        };
        ...
        public static CommandLineOptions Parse(string[] args)
        {
            ...
            // blueshot is a WinExe and has no console window by default.
        ...
        {
            var rootCommand = new RootCommand("blueshot")
            {
                Description = "blueshot is a free and open source screenshot tool for Windows.\n\n" +
                                 "Note: When another blueshot instance is already running, commands such as\n" +
                                 ...
                                 "instance. If no other instance is running, blueshot starts normally."
            };
