...
namespace blueshot.Helpers
{
    ...
    /// </summary>
    [Serializable]
    public class PluginHelper : IblueshotHost
    {
        ...
        private static readonly string PafPath = Path.Combine(Application.StartupPath, @"App\blueshot");

        public static PluginHelper Instance { get; } = new PluginHelper();

        public void Shutdown()
        {
            foreach (var plugin in SimpleServiceProvider.Current.GetAllInstances<IblueshotPlugin>())
            {
                plugin.Shutdown();
                plugin.Dispose();
            }
        ...
        public bool IsSelectedItemConfigurable(ListView listView)
        {
            ...
            var blueshotPlugin = (IblueshotPlugin) listView.SelectedItems[0].Tag;
            return blueshotPlugin?.IsConfigurable == true;
        }
        ...
        public void ConfigureSelectedItem(ListView listView)
        {
            ...
            var blueshotPlugin = (IblueshotPlugin) listView.SelectedItems[0].Tag;
            if (blueshotPlugin == null)
            {
                return;
            }

            var plugin = SimpleServiceProvider.Current
                .GetAllInstances<IblueshotPlugin>()
                .FirstOrDefault(p => p.Name == blueshotPlugin.Name);
            plugin?.Configure();
        }
        ...
        private IEnumerable<string> FindPluginsOnPath(string path)
        {
            ...
            try
            {
                pluginFiles = Directory.GetFiles(path, "blueshot.Plugin.*.dll", SearchOption.AllDirectories);
            }
            catch (Exception ex)
            {
        ...
        public void LoadPlugins()
        {
            ...
            if (blueshotEnvironment.IsPortable)
            {
            ...
            var plugins = new List<IblueshotPlugin>();
            foreach (string pluginFile in pluginFiles)
            {
                try
                {
                    ...
                    var pluginEntryName = $"{assemblyName}.{assemblyName.Replace("blueshot.Plugin.", string.Empty)}Plugin";
                    ...
                    var plugin = (IblueshotPlugin)Activator.CreateInstance(pluginEntryType);
                    if (plugin != null)
                    {
                        plugins.Add(plugin);
                    }
        ...
        private string GetPluginIdentifier(Assembly assembly, string pluginFile)
        {
            ...
            var pluginSubNamespace = assembly.GetName().Name.Replace("blueshot.Plugin.", string.Empty);
