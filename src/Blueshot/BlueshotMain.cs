...
public class blueshotMain
{
    ...
    public static void Main(string[] args)
    {
        ...
        var options = blueshotCommandLine.Parse(args);
        if (options == null)
        {
            return;
        }
        ...
        // Detect PortableApp (PAF) mode: the App\blueshot directory lives next to the executable.
        var startupPath = AppContext.BaseDirectory;
        var pafAppPath = Path.Combine(startupPath, @"App\blueshot");
        blueshotEnvironment.IsPortable = Directory.Exists(pafAppPath);

        // Build the IniConfigRegistry:
        //   AddAppDataPath  → %APPDATA%\blueshot
        //   AddSearchPath   → installation / startup directory
        //   --ini-directory → optional command-line override (highest priority)
        var builder = IniConfigRegistry.ForFile("blueshot.ini")
            .AddAppDataPath("blueshot")
        ...
        builder.AddDefaultsFile("blueshot-defaults.ini")
               .AddConstantsFile("blueshot-fixed.ini")
               ...
               .EmptyWhenNull()
               .LockFile()
               .EnableMetadata(applicationName: "blueshot");
