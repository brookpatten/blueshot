...
namespace blueshot.Forms
{
    ...
    public sealed partial class AboutForm : AnimatingBaseForm
    {
        ...
        public AboutForm()
        {
            ...
            lblTitle.Text = $@"blueshot {EnvironmentInfo.GetblueshotVersion()} {(blueshotEnvironment.IsPortable ? " Portable" : "")} ({OsInfo.Bits} bit) {(coreConfiguration.IsBetaTester ? "-IsBetaTester-":"")}";
            ...
            try
            {
                switch (keyData)
                {
                    ...
                    case Keys.L:
                        try
                        {
                            if (File.Exists(blueshotMain.LogFileLocation))
                            {
                                using (Process.Start("\"" + blueshotMain.LogFileLocation + "\""))
                                {
                            ...
                            {
                                MessageBox.Show(@"blueshot can't find the logfile, it should have been here: " + blueshotMain.LogFileLocation);
                            }
                        }
                        catch (Exception)
                        {
                            MessageBox.Show(@"Couldn't open the blueshot.log, it's located here: " + blueshotMain.LogFileLocation, @"Error opening blueshot.log",
                    ...
                    case Keys.I:
                        try
                        {
                            using (Process.Start("\"" + blueshotEnvironment.ConfigLocation + "\""))
                            {
                            }
                        }
                        catch (Exception)
                        {
                            MessageBox.Show(@"Couldn't open the blueshot.ini, it's located here: " + blueshotEnvironment.ConfigLocation, @"Error opening blueshot.ini",
                                MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                        }
