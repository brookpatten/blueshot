...
using blueshot.Base.Core.FileFormatHandlers;
using blueshot.Base.Help;
using blueshot.Base.Interfaces;
using blueshot.Base.Interfaces.Ocr;
using blueshot.Configuration;
using blueshot.Controls;
using blueshot.Destinations;
using blueshot.Editor;
using blueshot.Editor.Destinations;
using blueshot.Editor.Drawing;
using blueshot.Editor.Forms;
using blueshot.Helpers;
using blueshot.Plugin.Win10;
using blueshot.Processors;
using log4net;
...
{
    /// <summary>
    /// This is the MainForm, the shell of blueshot
    /// </summary>
    public partial class MainForm : BaseForm, IblueshotMainForm, ICaptureHelper, IProvideDeviceDpi
    {
        ...
        /// <summary>
        /// Application entry-point, called from <see cref="blueshotMain"/> after the
        ...
        public static void Start(CommandLineOptions options)
        {
            try
            {
                ...
                _applicationMutex = ResourceMutex.Create("F48E86D3-E34C-4DB7-8F8F-9A0EA55F0D08", "blueshot", false);
                ...
                if (isAlreadyRunning)
                {
                    ...
                    else
                    {
                        ...
                        foreach (Process blueshotProcess in Process.GetProcessesByName("blueshot"))
                        {
                            try
                            {
                                instanceInfo.Append(index++ + ": ").AppendLine(Kernel32Api.GetProcessPath(blueshotProcess.Id));
                                if (currentProcessId == blueshotProcess.Id)
                                {
                        ...
                        // A dirty fix to make sure the message box is visible as a blueshot window on the taskbar
                        using Form dummyForm = new Form
                        {
                            Icon = blueshotResources.GetblueshotIcon(),
                            ShowInTaskbar = true,
                ...
                // From here on we continue starting blueshot
            ...
            catch (Exception ex)
            {
                ...
                blueshotMain.Application_ThreadException(ActiveForm, new ThreadExceptionEventArgs(ex));
            }
        }
        ...
        public MainForm(CommandLineOptions options)
        {
            ...
            SimpleServiceProvider.Current.AddService<IblueshotMainForm>(this);
            ...
            notifyIcon.Icon = blueshotResources.GetblueshotIcon();
            ...
            // This forces the registration of all destinations inside blueshot itself.
            RegisterInternalDestinations();
            // This forces the registration of all processors inside blueshot itself.
            ...
            // Set the blueshot icon visibility depending on the configuration. (Added for feature #3521446)
            ...
            _copyData.Channels.Add("blueshot");
            ...
            // Make blueshot use less memory after startup
        ...
        /// <summary>
        /// Context menu entry "Support blueshot"
        /// </summary>
        /// <param name="sender">object</param>
        ...
        private void Contextmenu_DonateClick(object sender, EventArgs e)
        {
            Dispatcher.CurrentDispatcher.BeginInvoke(() =>
            {
                Process.Start("https://getblueshot.org/support/?version=" + EnvironmentInfo.GetblueshotVersion(true));
            });
        }
        ...
        /// <summary>
        /// The "About blueshot" entry is clicked
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
