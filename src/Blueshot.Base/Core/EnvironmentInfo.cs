...
namespace blueshot.Base.Core
{
    ...
    public static class EnvironmentInfo
    {
        ...
        public static string GetblueshotVersion(bool shortVersion = false)
        {
            var executingAssembly = Assembly.GetExecutingAssembly();

            // Use assembly version
            string blueshotVersion = executingAssembly.GetName().Version.ToString();
            ...
            if (!string.IsNullOrEmpty(assemblyFileVersionAttribute?.Version))
            {
                ...
                blueshotVersion = assemblyFileVersion.ToString(2);
                try
                {
                    blueshotVersion = assemblyFileVersion.ToString(3);
                }
                catch (Exception)
                {
                    // Ignore
            ...
            if (!shortVersion)
            {
                ...
                {
                    blueshotVersion = informationalVersionAttribute.InformationalVersion;
                }
            }

            return blueshotVersion.Replace("+", " - ");
        }

        public static string EnvironmentToString(bool newline)
        {
            StringBuilder environment = new();
            environment.Append("Software version: " + GetblueshotVersion());
            ...
            environment.Append(string.Join(",", SimpleServiceProvider.Current.GetAllInstances<IblueshotPlugin>().Select(plugin => plugin.Name)));
