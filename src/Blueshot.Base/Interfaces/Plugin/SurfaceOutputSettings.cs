...
namespace blueshot.Base.Interfaces.Plugin
{
    public class SurfaceOutputSettings
    {
        ...
        /// <summary>
        /// BUG-2056 reported a logical issue, using blueshot format as the default causes issues with the external commands.
        /// </summary>
        /// <returns>this for fluent API usage</returns>
        public SurfaceOutputSettings PreventblueshotFormat()
        {
            // If OutputFormat is blueshot, use PNG instead.
            if (Format == OutputFormat.blueshot)
            {
